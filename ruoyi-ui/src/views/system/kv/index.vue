<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="kvList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="Value" align="center" prop="json" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="Value" prop="json">
          <el-input v-model="form.json" placeholder="请输入Value" />
        </el-form-item>
      </el-form>
...
<script>
import { listKv, getKv, delKv, addKv, updateKv, exportKv } from "@/api/system/kv";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除键值对编号为"' + ids + '"的数据项?', "警告", {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有键值对数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(() => {
          this.exportLoading = true;
          return exportKv(queryParams);
        }).then(response => {
